...
class Main:
    ...
    def save_page(self):
        ...
        output_from_parsed_template = self.template.render(name=self.name, most_active=most_active, runner_ups=runner_ups, being=being, urls_used=most_urls, total=total_num, activity_graph=self.activity_graph) # All necessary variables goes here
        ...
        with open("/var/www/alltime/json/%s.json" % self.name, "w") as fh:
            json.dump({"name": self.name, "total_numbers": total_num, "most_active": most_active, "runner_ups": runner_ups, "being": being, "most_used_urls": most_urls, "activity_graph": self.activity_graph}, fh)
